...
Microsoft.Reporting.WebFormsClient._ToolbarImageHolder = function (element)
{
    Microsoft.Reporting.WebFormsClient._ToolbarImageHolder.initializeBase(this, [element]);
...
Microsoft.Reporting.WebFormsClient._ToolbarImageHolder.prototype =
{
    initialize: function ()
    {
        Microsoft.Reporting.WebFormsClient._ToolbarImageHolder.callBaseMethod(this, "initialize");
        $addHandlers(this.get_element(), { "click": this.OnClick }, this);
    },
    ...
    dispose: function ()
    {
        ...
        Microsoft.Reporting.WebFormsClient._ToolbarImageHolder.callBaseMethod(this, "dispose");
    },
    ...
    IsButtonDisabled: function ()
    {
        ...
        // Button is table element. The image renders Enabled=false as disabled="disabled" attribute.
...
Microsoft.Reporting.WebFormsClient._ToolbarImageHolder.registerClass("Microsoft.Reporting.WebFormsClient._ToolbarImageHolder", Sys.UI.Control);// <copyright company="Microsoft">
//     Copyright (c) Microsoft.  All rights reserved.
// </copyright>
...
Microsoft.Reporting.WebFormsClient._HoverImage.registerClass("Microsoft.Reporting.WebFormsClient._HoverImage", Microsoft.Reporting.WebFormsClient._ToolbarImageHolder);// <copyright company="Microsoft">
//     Copyright (c) Microsoft.  All rights reserved.
// </copyright>

Type.registerNamespace("Microsoft.Reporting.WebFormsClient");

Microsoft.Reporting.WebFormsClient._StickyImage = function (element)
{
    Microsoft.Reporting.WebFormsClient._StickyImage.initializeBase(this, [element]);
}

Microsoft.Reporting.WebFormsClient._StickyImage.prototype =
{
    initialize: function () {
        Microsoft.Reporting.WebFormsClient._StickyImage.callBaseMethod(this, "initialize");

        _$RVCommon.setButtonStyle(this.get_element(), this._hoverStyles, "pointer");
    },

    dispose: function () {
        Microsoft.Reporting.WebFormsClient._StickyImage.callBaseMethod(this, "dispose");
    },
...
Microsoft.Reporting.WebFormsClient._StickyImage.registerClass("Microsoft.Reporting.WebFormsClient._StickyImage", Microsoft.Reporting.WebFormsClient._ToolbarImageHolder);
// <copyright company="Microsoft">
//     Copyright (c) Microsoft.  All rights reserved.
// </copyright>
...
Microsoft.Reporting.WebFormsClient._InternalReportViewer = function(element)
{
    ...
    this.DownloadLinkText = null;
    this.LoadingIconUrl = null;
    this.PageSizeLabelText = null;
    this.PageOrientationLabelText = null;
    this.LandscapeText = null;
    this.PortraitText = null;

    this.m_PageSizeList = null;
    ...
    var _this = this;
    this.m_OnWindowResizeDelegate = function () {

        var reportViewer = $get(_this.ReportViewerId);
        if (reportViewer && _this.BaseHeight.indexOf('%') >= 0) {
            reportViewer.style.height = _this.BaseHeight;
        }
...
Microsoft.Reporting.WebFormsClient._InternalReportViewer.prototype =
{
    ...
    set_PageSizeList: function (value) { this.m_PageSizeList = value;},
    get_PageSizeList: function () { return this.m_PageSizeList;},
    ...
    HideSizingRow: function()
    {
        ...
        // VSTS 4657883: Previous change did not work for IE10Compat Browser with Quirks Doc mode. Using an 
        // existing JS function that correctly determines browser type for that case.
        if (this.HasSizingRow && _$RVCommon.isIEQuirksMode() && _$RVCommon.isPreIE8StandardsMode()) {
    ...
    OnReportAreaNewContentVisible: function(sender, eventArgs)
    {
        ...
        {
            if (RSTelemetry) {
                RSTelemetry.trackEvent("RS.ReportViewer.Render", {});
            }
    ...
    ResizeViewerReportUsingContainingElement: function(forceRecalculate)
    {
        ...
        var availableReportViewerHeight = window.$RSjQuery(reportViewer).innerHeight(); // Need to use the innerHeight as the Viewer Control border etc. will take up space
        reportAreaHeight = availableReportViewerHeight - toolbarHeight;
        ...
        if (reportArea.control.GetReportPage().IsReportViewerInVs()) {
            var reportControl = window.$RSjQuery(reportViewer);
            var borderWidth = reportControl.outerWidth() - reportControl.innerWidth();
            var calculatedHeight = reportAreaHeight - borderWidth;
            window.$RSjQuery(reportArea).height(calculatedHeight);
        }
        else {
            window.$RSjQuery(reportArea).height(window.$RSjQuery(window).height() - window.$RSjQuery(reportArea).offset().top);
        }
    },
    ...
    ExportReport: function(format)
    {
        ...
        if (RSTelemetry) {
            RSTelemetry.trackEvent("RS.ReportViewer.Export", {
                format: format
            });
        }
    ...
    PdfPrint: function ()
    {
        if (this.PdfDownloadUrl == null)
            return false;
        var $ = window.$RSjQuery || window.$;
        // Url and Text Strings
        var pdfPreviewUrl = this.PdfPreviewUrl;
        var pdfDownloadUrl = this.PdfDownloadUrl;
        var loadingText = this.LoadingText;
        var footerText = this.FooterText;
        var captionText = this.CaptionText;
        var printText = this.PrintText;
        var doneText = this.DoneText;
        var downloadText = this.DownloadText;
        var downloadLinkText = this.DownloadLinkText;
        var loadingIconUrl = this.LoadingIconUrl;
        var settingText = this.SettingText;
        var pageSizeLabelText = this.PageSizeLabelText;
        var pageOrientationLabelText = this.PageOrientationLabelText;
        var landscapeText = this.LandscapeText;
        var portraitText = this.PortraitText;

        // Elements IDs
        var iframePDFPreviewClass = ".msrs-printdialog-pdfiframe";
        var iframePDFPreview = "#" + this.ReportViewerId + " " + iframePDFPreviewClass;
        var divPDFContainer = "#" + this.ReportViewerId + " .msrs-printdialog-pdfcontainer";
        var divPrinting = "#" + this.ReportViewerId + " .msrs-printdialog-printing";
        var divPrintDialog = "#" +this.ReportViewerId + " .msrs-printdialog-main";
        var divDialogBody = "#" +this.ReportViewerId + " .msrs-printdialog-body";
        var divLoading = "#" +this.ReportViewerId + " .msrs-printdialog-loading";
        var pLoadingText = "#" + this.ReportViewerId + " .msrs-printdialog-loadingtext";
        var pCaptionClass = ".msrs-printdialog-caption";
        var pCaption = "#" + this.ReportViewerId + " " + pCaptionClass;
        var aDownloadLink = "#" + this.ReportViewerId + " .msrs-printdialog-downloadlink";
        var pDialogText = "#" + this.ReportViewerId + " .msrs-printdialog-bodytext";
        var divPrintOverlay = ".msrs-printdialog-overlay"; // it fades the entire body , so the scope of the selector is not the report control div
        var spanClose = "#" + this.ReportViewerId + " .msrs-printdialog-closebutton";
        var imgLoadingIcon = "#" +this.ReportViewerId + " .msrs-printdialog-loadingicon";
        var divSetting = "#" +this.ReportViewerId + " .msrs-printdialog-setting";
        var pSettingText = "#" + this.ReportViewerId + " .msrs-printdialog-settingtext";
        var divPrintButton = "#" +this.ReportViewerId + " .msrs-printdialog-divprintbutton";
        var divCancelButton = "#" + this.ReportViewerId + " .msrs-printdialog-divcancelbutton";
        var divPageSize = "#" +this.ReportViewerId + " .msrs-printdialog-divpagesize";
        var divPageOrientation = "#" +this.ReportViewerId + " .msrs-printdialog-divpageorientation";
        var listPageSize = "#" + this.ReportViewerId + " .msrs-printdialog-list-pagesize";
        var listPageOrientation = "#" + this.ReportViewerId + " .msrs-printdialog-list-pageorientation";
        var labelPageSize = "#" + this.ReportViewerId + " .msrs-printdialog-label-pagesize";
        var labelPageOrientation = "#" + this.ReportViewerId + " .msrs-printdialog-label-pageorientation";
        var selectedOptionSelector = "#" + this.ReportViewerId + " option:selected";

        var preDialogFocus = window.$RSjQuery(":focus");

        var fadeTime = 300;
        var pageSizes = this.m_PageSizeList.PageSizes;
        ...
        if (window.$RSjQuery(divPrintDialog).css("display") != "none") {
            return false;
        }

        if (window.$RSjQuery(divPrintOverlay).length == 0) {
            window.$RSjQuery("<div />", {
                "class": divPrintOverlay.substring(1)
            }).appendTo("body");
        }
        ...
        var GetBrowser = function ()
        {
            ...
            {
                browser = "IE";
                versionRegexp = /MSIE (\d+\.\d+);/;
            }
            else if (uAg.match(/Trident/i))
            {
                browser = "IE";
                versionRegexp = /Trident.*rv[ :]*(\d+\.\d+)/;
            }
            else if (uAg.match(/Edge/i))
            {
                browser = "Edge";
                versionRegexp = /Edge[\/\s](\d+\.\d+)/;
            }
            else if (uAg.match(/Firefox/i))
            {
                browser = "FireFox";
                versionRegexp = /Firefox[\/\s](\d+\.\d+)/;
            }
            else if (uAg.match(/Chrome/i))
            {
                browser = "Chrome";
                versionRegexp = /Chrome[\/\s](\d+\.\d+)/;
            }
            else if (uAg.match(/Safari/i))
            {
                browser = "Safari";
                versionRegexp = /Safari[\/\s](\d+\.\d+)/;
            }
            else
            {
                browser = "Other";
        ...
        //Load page size items into the page setting drop-down menu
        var ConvertToInches = function(value, unit) {
        ...
        var HaveSameLength = function(length1, length2) {
            var epsilon = 0.0101;
            return (Math.abs(length1 - length2) < epsilon)?true:false; 
        }

        var MatchDefaultPageSize = function() {
            var defaultHeight = pageSizes[0].Height;
            var defaultWidth = pageSizes[0].Width;
            var matchedItem = 0;
            for (var i = 1; i < pageSizes.length; i++)
            {
                if ((HaveSameLength(defaultHeight, ConvertToInches(pageSizes[i].Height, pageSizes[i].Unit))) && (HaveSameLength(defaultWidth, ConvertToInches(pageSizes[i].Width, pageSizes[i].Unit)))) {
                    matchedItem = i;
                    break;
                }
            }
        ...
        var EnterKeydown = function(event) {
            if (event.keyCode==13) {
                event.preventDefault();
                event.stopPropagation();
                window.$RSjQuery(divPrintButton).click();
            }
        }

        var InitPageSetting = function() {

            window.$RSjQuery(labelPageSize).text(pageSizeLabelText);
            window.$RSjQuery(labelPageOrientation).text(pageOrientationLabelText);

            var matchedDefault = MatchDefaultPageSize();
            if (matchedDefault == 0)
            {
                window.$RSjQuery(listPageSize).append(window.$RSjQuery('<option>', {
                    value: 0,
                    text: pageSizes[0].OptionText
                }));
            }
            for (var i = 1; i < pageSizes.length; i++) {
                window.$RSjQuery(listPageSize).append(window.$RSjQuery('<option>', {
                    value: i,
                    text: pageSizes[i].OptionText
                }));
            }

            window.$RSjQuery(listPageSize).val(matchedDefault);
            pageSetting.width = pageSizes[matchedDefault].Width + pageSizes[matchedDefault].Unit;
            pageSetting.height = pageSizes[matchedDefault].Height + pageSizes[matchedDefault].Unit;
            pageSetting.aspectRatio = pageSizes[matchedDefault].Height / pageSizes[matchedDefault].Width;
            
            window.$RSjQuery(listPageSize).change(function () {
                var indexSelected = parseInt(window.$RSjQuery(listPageSize + selectedOptionSelector).val());
                pageSetting.width = pageSizes[indexSelected].Width + pageSizes[indexSelected].Unit;
                pageSetting.height = pageSizes[indexSelected].Height + pageSizes[indexSelected].Unit;
                pageSetting.aspectRatio = pageSizes[indexSelected].Height / pageSizes[indexSelected].Width;
            });

            window.$RSjQuery(divPageSize).keydown(EnterKeydown);

            window.$RSjQuery(listPageOrientation).append(window.$RSjQuery('<option>', {
                value: 0,
                text: portraitText
            }));
            ...
            if (pageSizes[0].IsLandscape)
            {
                window.$RSjQuery(listPageOrientation).val(1);
                pageSetting.orientation = "landscape";
            }
            else
            {
                window.$RSjQuery(listPageOrientation).val(0);
                pageSetting.orientation = "portrait";
            }

            window.$RSjQuery(listPageOrientation).change(function () {
                var selected = parseInt(window.$RSjQuery(listPageOrientation + selectedOptionSelector).val());
                pageSetting.orientation = (selected == 0) ? "portrait" : "landscape";
            });
            window.$RSjQuery(divPageOrientation).keydown(EnterKeydown);
        }

        //Called when the iframe is fully loaded, hide the loading icon and show "Done printing". Works in Chrome only.
        var FinishLoading = function ()
        {
            window.$RSjQuery(divLoading).hide();
            window.$RSjQuery(divDialogBody).show();
            window.$RSjQuery(divPrintDialog).css({ top: ((window.outerHeight - window.$RSjQuery(divPrintDialog).height()) / 2), left: ((window.outerWidth - window.$RSjQuery(divPrintDialog).width()) / 2) });
        }

        //There are multiple ways to close the print dialog. This method is called when the dialog is closed.
        //PDF iframe and gray-out overlay are removed. Page setting drop-down menu is cleared.
        var ClosePrintDialog = function ()
        {
            window.$RSjQuery(window).off("resize");
            window.$RSjQuery(divPrintButton).off("click");
            window.$RSjQuery(divCancelButton).off("click");
            window.$RSjQuery(spanClose).off("click");
            window.$RSjQuery(divPrintOverlay).off("click");
            window.$RSjQuery(document).off("keydown");
            window.$RSjQuery(divPageSize).off("keydown");
            window.$RSjQuery(divPageOrientation).off("keydown");
            window.$RSjQuery(listPageSize).empty();
            window.$RSjQuery(listPageOrientation).empty();
            window.$RSjQuery(divPrintDialog).fadeOut(fadeTime, function() { window.$RSjQuery(divPDFContainer).empty(); });
            window.$RSjQuery(divPrintOverlay).fadeOut(fadeTime, function () { window.$RSjQuery(divPrintOverlay).remove(); });
            preDialogFocus.focus();
        }

        //This method is called after user specified the page setting. The corresponding pdf is requested based on the page setting, for both iframe and pdf download.
        var ProcessPrint = function (p_pageSetting)
        {
            var urlPageExt;

            if (p_pageSetting.orientation === "landscape")
            {
                urlPageExt = "&rc:PageWidth=" + p_pageSetting.height + "&rc:PageHeight=" + p_pageSetting.width;
                p_pageSetting.aspectRatio = 1 / p_pageSetting.aspectRatio;
            }
            else
            {
                urlPageExt = "&rc:PageWidth=" + p_pageSetting.width + "&rc:PageHeight=" + p_pageSetting.height;
            }

            window.$RSjQuery(aDownloadLink).attr("href", pdfDownloadUrl + urlPageExt);
            window.$RSjQuery(aDownloadLink).text(downloadLinkText);
            window.$RSjQuery(pDialogText).text(dialogText);
            
            if (RSTelemetry) {
                RSTelemetry.trackEvent("RS.ReportViewer.Print", {
                    orientation: p_pageSetting.orientation,
                    height: p_pageSetting.height,
                    width: p_pageSetting.width
                });
            }
            ...
            if (inNewTab)
            {
                window.$RSjQuery(divDialogBody).show();
                window.$RSjQuery(divLoading).hide();
                window.open(pdfPreviewUrl + urlPageExt, "_blank");
            }
            else
            {
            
                window.$RSjQuery(divPDFContainer).empty();

                window.$RSjQuery("<iframe />", {
                    "class": iframePDFPreviewClass.substring(1),
                    "src": pdfPreviewUrl + urlPageExt,
                    "width": "100%",
                    "height": "200px"
                }).appendTo(divPDFContainer);

                if (!showPreview)
                {
                    window.$RSjQuery(iframePDFPreview).hide();
                }

                if (showLoading)
                {
                    window.$RSjQuery(divDialogBody).hide();
                    window.$RSjQuery(divLoading).show();
                    window.$RSjQuery(pLoadingText).text(loadingText);
                    window.$RSjQuery(imgLoadingIcon).attr("src", loadingIconUrl);
                    window.$RSjQuery(iframePDFPreview).on("load", FinishLoading);
                }
                else
                {
                    window.$RSjQuery(divDialogBody).show();
        ...
        var SettingDialogKeyDown = function (event)
        {
            switch (event.keyCode) {
                //enter
                case 13:
                    event.preventDefault();
                    if (window.$RSjQuery(event.target).is(window.$RSjQuery(divPrintDialog))) {
                        if (window.$RSjQuery(divPrinting).css("display") == "none") {
                            window.$RSjQuery(divPrintButton).click();
                        }
                    } else {
                        window.$RSjQuery(event.target).click();
                    }
                    break;
                //escape
                case 27:
        ...
        var PrintDialogKeyDown = function (event)
        {
            if (event.keyCode==27)
            {
                ClosePrintDialog();
                ...
                if (window.$RSjQuery(event.target).is(window.$RSjQuery(spanClose))) {
                    event.preventDefault();
                    ClosePrintDialog();
                }
            }
        ...
        var printDialogTabItems = window.$RSjQuery("#" + this.ReportViewerId).find(".msrs-printdialog-main *[tabindex], .msrs-printdialog-main select");
        var TabCatch = function (event) {
            if (event.keyCode === 9) {
                var focusedItem;
                var visiblePrintDialogTabItems = printDialogTabItems.filter(":visible");
                focusedItem = window.$RSjQuery(':focus');
                var focusedItemIndex = visiblePrintDialogTabItems.index(focusedItem);
                if (focusedItemIndex === -1) {
                    visiblePrintDialogTabItems[0].focus();
                    event.preventDefault();
                } else {
                    if (event.shiftKey && focusedItemIndex === 0) {
                        visiblePrintDialogTabItems[visiblePrintDialogTabItems.length - 1].focus();
                        event.preventDefault();
                    } else if (!event.shiftKey && focusedItemIndex === visiblePrintDialogTabItems.length - 1) {
                        visiblePrintDialogTabItems[0].focus();
                        event.preventDefault();
                    }
                }
        ...
        window.$RSjQuery(document).keydown(TabCatch);

        //Show print dialog with page setting
        var ShowPrintDialog = function () {

            window.$RSjQuery(pCaption).text(captionText);
            window.$RSjQuery(pSettingText).text(settingText);
            window.$RSjQuery(divPrinting).hide();
            window.$RSjQuery(divSetting).show();

            InitPageSetting();

            window.$RSjQuery(divPrintButton).click(function ()
            {
                window.$RSjQuery(divPrinting).show();
                window.$RSjQuery(divSetting).hide();
                ProcessPrint(pageSetting);
                window.$RSjQuery(document).off("keydown");
                window.$RSjQuery(document).keydown(PrintDialogKeyDown);
            });

            window.$RSjQuery(document).keydown(SettingDialogKeyDown);

            window.$RSjQuery(divPrintOverlay).fadeIn(fadeTime);
            window.$RSjQuery(divPrintDialog).css({ top: ((window.outerHeight - window.$RSjQuery(divPrintDialog).height()) / 2), left: ((window.outerWidth - window.$RSjQuery(divPrintDialog).width()) / 2) });
            window.$RSjQuery(divPrintDialog).fadeIn(fadeTime);
            window.$RSjQuery(divPrintDialog).focus();
        }

        this.ShowPrintDialog = ShowPrintDialog;

        if (window.$RSjQuery.ui) {
            window.$RSjQuery(divPrintDialog).draggable({
                start: function () {
                    window.$RSjQuery(iframePDFPreview).css("pointer-events", "none");
                },
                stop: function () {
                    window.$RSjQuery(iframePDFPreview).css("pointer-events", "auto");
                },
                handle: pCaptionClass,
                containment: "window",
                scroll: false
            });
        }
        ...
        {
            window.$RSjQuery(divPrintOverlay).css({ top: 0, left: 0, width: "100%", height: "100%" });
            window.$RSjQuery(divPrintDialog).css({ top: ((window.outerHeight - window.$RSjQuery(divPrintDialog).height()) / 2), left: ((window.outerWidth - window.$RSjQuery(divPrintDialog).width()) / 2) });
        });

        window.$RSjQuery(spanClose).click(ClosePrintDialog);

        window.$RSjQuery(divPrintOverlay).click(ClosePrintDialog);

        window.$RSjQuery(divCancelButton).click(ClosePrintDialog);

        //Based on the browser and plugin, determine the print dialog behavior
        var currentBrowser = GetBrowser();
        var pluginName = GetPdfPluginName();

        var showPreview = false;
        var showLoading = false;
        var showFooter = false;
        var inNewTab = false;
        var dialogText = downloadText;

        if (currentBrowser.browser.match(/Edge/i))
        {
            inNewTab = true;
            dialogText = doneText;
        }
        else if (!pluginName.match(/NoPDFPlugin/i))
        {
            showFooter = true;
            if ((currentBrowser.browser.match(/IE/i)) && (pluginName.match(/Adobe/i) || pluginName.match(/Foxit/i)))
            {
                showPreview = true;
                dialogText = printText;
            }
            else if ((currentBrowser.browser.match(/Chrome/i)) && pluginName.match(/Chrome/i))
            {
                showLoading = true;
                dialogText = doneText;
                ...
                {
                    // If server is earlier version than 2016, fallback to IE workflow
                    if (this.ServerVersion.slice(0,4) < 2015)
                    {
                        showPreview = true;
                        dialogText = printText;
                        showLoading = false;
            ...
            else if ((currentBrowser.browser.match(/FireFox/i)) && (currentBrowser.version > 19))
            {
                showPreview = true;
                dialogText = printText;
            }
        }
        else if (currentBrowser.browser.match(/Firefox/i)) {
            inNewTab = true;
            dialogText = doneText;
        }
    ...
    _UpdateDocMapAreaUIVisibility: function(hideEverything)
    {
        ...
        // Document container visibility for screen readers
        var docMapContainer = docMapCells.docMapContainer;
        if (docMapContainer)
        {
            if (isVisible)
            {
                docMapContainer.removeAttribute("hidden");
            }
            else
            {
                // Hiding container from screen readers
                docMapContainer.setAttribute("hidden", "true");
            }
        }
    ...
    SetDocMapAreaHeight: function(reportAreaHeight)
    {
        if (this.DocMapAreaId != null)
        {
            ...
            if (docMapCells.docMapContainerCell != null)
            {
                ...
                var docMapTitleHeight = docMapCells.docMapTitleCell.offsetHeight;
...
Microsoft.Reporting.WebFormsClient._ValidValueParameterInputControl.prototype = 
{
    ...
    GetCurrentParameterValues: function () {
        var dropDown = $get(this.DropDownId);
...
Microsoft.Reporting.WebFormsClient._GenericDropDownParameterInputControl.prototype = 
{
    ...
    SelectImage : function(useEnabledImage)
    {
        ...
        if (useEnabledImage)
        {
            ...
            inputCtrl.classList.remove('disabled');
            inputCtrl.style.cursor = 'pointer';
        }
        else
        {
            inputCtrl.src = this.DisabledImageSrc;
            inputCtrl.classList.add('disabled');
    ...
    GetDropDownPosition : function()
    {
        ...
        // This ParentDiv is the Scroll container. This is to fix the multivalue dropdown position
        var scrollDiv = $get(this.ParentDivID);
        return { Left: textBoxPosition.Left - scrollDiv.scrollLeft, Top: (textBoxPosition.Top + visibleTextBox.offsetHeight) - scrollDiv.scrollTop };
    },
...
Microsoft.Reporting.WebFormsClient._MultiValueParameterInputControl.prototype =
{
    ...
    GetCurrentValueFromTextEditor: function()
    {
        ...
        if (!this.AllowBlank)
        {
            ...
            finalValue = valueString.replace(new RegExp(this.TextAreaDelimiter, "g"), this.ListSeparator);
        }

        this.m_parameterValues = finalValue.split(this.ListSeparator);
    ...
    GetCurrentValueFromValidValueList: function()
    {
        ...
        for (var i = startIndex; i < length; i++)
        {
            ...
            {
                valueString.push(this.Trim(rowInfo.Label));
                indexString.append((i - startIndex).toString());
                this.m_parameterValues.push(i);
            }
        }
...
Microsoft.Reporting.WebFormsClient._PromptArea.prototype =
{
    ...
    GetParameterCollection: function () 
    {
        var paramCollection = {};
        if (this.m_parameterIdList) {
            for (var i = 0; i < this.m_parameterIdList.length; i++) {
                ...
                var parameterControl = parameterElement.control;
                var val;
                if (parameterControl.NullCheckBoxId && $get(parameterControl.NullCheckBoxId).checked) {
                    val = null;
                }
                else if (parameterControl.constructor.getName() === "Microsoft.Reporting.WebFormsClient._ValidValueParameterInputControl") {
                    val = parameterControl.GetCurrentParameterValues();
                } else if (parameterControl.constructor.getName() === "Microsoft.Reporting.WebFormsClient._MultiValueParameterInputControl") {
                    val = parameterControl.m_parameterValues;
                } else {
                    val = parameterControl.GetCurrentValue();
                }
                var name = parameterElement.getAttribute("data-parameterName");
                paramCollection[name] = val;
            }
        }
    ...
    OnViewReportClick: function(e) 
    {
        ...
        var eventData = {
            event:  "OnViewReportClickEvent",
            message: this.GetParameterCollection()
        };

        // send a message to parent to inform about report action
        ...
        if (this.CanScrollReportArea())
        {
            ...
            if (reportPage.NavigationId != null)
            {
                ...
                else
                {
                    ...
                    if (reportPage.CurrentViewNavigationFocusId != null)
                    {
                        var targetElement = $get(reportPage.CurrentViewNavigationFocusId);
...
Microsoft.Reporting.WebFormsClient._ReportPage = function(element)
{
    ...
    this.CurrentViewNavigationFocusId = null;
...
Microsoft.Reporting.WebFormsClient._ReportPage.prototype =
{
    ...
    OnReportVisible: function ()
    {
        ...
        // HTML5 renderer specific actions, won't be available in the HTML4 renderer
        if (PostRenderActions !== undefined) {
            PostRenderActions();
        }
    },
    ...
    InvokeReportAction: function (actionType, actionParam, event)
    {
        if (!this._IsInputDisabled())
        {
            ...
            var urlAttr = event ? event.currentTarget.attributes["data-drillThroughUrl"] : null;
            if (urlAttr && urlAttr.value) {
                var eventData = {
                    event: "InvokeReportActionEvent",
                    message: urlAttr.value
                };
    ...
    HighlightNextSearchHit: function ()
    {
        ...
        while (hitElem != null)
        {
            hitElem.className = "searchHighlighting";
            hitElem = $get(name + "_" + (++count));
        }
    ...
    _ApplyZoom: function (zoomValue)
    {
        ...
        if ((zoomValue != this.ConstPageWidth) && (zoomValue != this.ConstFullPage)) {
            reportCell.style.transform = "scale(" + zoomValue / 100 + ")";
            reportCell.style.transformOrigin = "0 0";
        }
        else {
        ...
        if (!this.IsReportViewerInVs()) {
            this.FixPortalScrollBars();
        }
        this.OnScroll();
    },

    IsReportViewerInVs: function () {
        var $ = window.$RSjQuery || window.$;
        if ($("[id$='_isReportViewerInVs']") && $("[id$='_isReportViewerInVs']").length > 0)
            return true;
        else
            return false;
    },

    FixPortalScrollBars: function () {
        var $ = window.$RSjQuery || window.$;
        if ($) {
            $('div[id*="VisibleReportContent"]').each(
                function () {
                    var parent = $(this).parent();
                    if (parent)
                        parent.height($(window).height() -parent.offset().top);
                });
        }
    },
    ...
    _ApplyCalculatedZoom: function (reportCell, div, rep)
    {
        if (rep <= 0)
            return 1.0;
        var z = (div - 1) / rep;
        reportCell.style.transform = "scale(" + z + ")";
        reportCell.style.transformOrigin = "0 0";
        return z;
    },
    ...
    set_zoomLevel: function (newZoomLevel)
    {
        ...
        // Chrome is not recalculating the real height of the report area after the zoom factor is applied
        // Which results in the scroll bars not being the correct height. Forcing recalculation by modifying the DOM
        $($get(this.ReportCellId)).parent().hide().show(0);
    },
...
Microsoft.Reporting.WebFormsClient._ReportPage.registerClass("Microsoft.Reporting.WebFormsClient._ReportPage", Sys.UI.Control);// <copyright company="Microsoft">
//     Copyright (c) Microsoft.  All rights reserved.
// </copyright>
...
Microsoft.Reporting.WebFormsClient._ScriptStickyImage = function (element) {
    Microsoft.Reporting.WebFormsClient._ScriptStickyImage.initializeBase(this, [element]);
...
Microsoft.Reporting.WebFormsClient._ScriptStickyImage.prototype =
{
    ...
    set_Image3: function (value) { this.m_image3 = value; },
    get_Image3: function () { return this.m_image3; },

    dispose: function () {
        this.m_image1 = null;
        this.m_image2 = null;
        this.m_image3 = null;
        Microsoft.Reporting.WebFormsClient._ScriptStickyImage.callBaseMethod(this, "dispose");
    },

    ShowImage: function (shouldShowImage1) {
        if (this.m_image1 == null || this.m_image2 == null || this.m_image3 == null)
            return;

        if (shouldShowImage1) {
            this.m_image1.style.display = "";
            this.m_image2.style.display = "none";
            this.m_image3.style.display = "none";
        }
        else {
    ...
    SetOnClickHandler: function (press, handler) {
        if (press)
            this.m_image1.control.OnClickScript = handler;
        else
            this.m_image3.control.OnClickScript = handler;
    }
}

Microsoft.Reporting.WebFormsClient._ScriptStickyImage.registerClass("Microsoft.Reporting.WebFormsClient._ScriptStickyImage", Sys.UI.Control);
// <copyright company="Microsoft">
//     Copyright (c) Microsoft.  All rights reserved.
// </copyright>
...
Microsoft.Reporting.WebFormsClient._Toolbar = function (element)
{
    ...
    // Power BI features
    this.m_powerBIFeaturesButton = null;
    this.m_onPowerBIFeaturesPressDelegate = Function.createDelegate(this, this.OnPowerBIFeaturesPress);
...
Microsoft.Reporting.WebFormsClient._Toolbar.prototype =
{
    initialize: function ()
    {
        ...
        var d = new Date();
        var currentSeconds = d.getTime() / 1000;
        var timeout = currentSeconds + 15;

        // This was added for RFC 5640019 - if this.m_reportViewer doesn't exist, wait 15 seconds for it to be populated.
        while (!this.m_reportViewer && currentSeconds < timeout) {
            d = new Date();
            currentSeconds = d.getTime() / 1000;
        }
    ...
    dispose: function ()
    {
        ...
        if (this.m_onFindTextChangeDelegate != null)
        {
            ...
            delete this.m_onPowerBIFeaturesPressDelegate;
            this.m_onPowerBIFeaturesPressDelegate = null;
    ...
    set_PowerBIFeaturesButton: function (value) { this.m_powerBIFeaturesButton = value; },
    get_PowerBIFeaturesButton: function () { return this.m_powerBIFeaturesButton; },
    ...
    ResizeToolbarIcons: function () {
        var $ = window.$RSjQuery || window.$;
        var reportViewer = window.$RSjQuery("#" + this.m_reportViewer._id);

        // Create RTL style to customize styling in RTL environments (ie. flip icons)
        if (window.$RSjQuery(reportViewer).css('direction').toUpperCase() === 'RTL') {
            window.$RSjQuery(reportViewer).addClass('rsRtl');
        } else {
            window.$RSjQuery(reportViewer).addClass('rsLtr');
        }

        var availableSpace = window.$RSjQuery(reportViewer).width();
        var toolbarButtons = window.$RSjQuery(reportViewer).find(".MenuBarBkGnd > .ToolBarButtonsCell").children();

        // Reset everything
        window.$RSjQuery(reportViewer).find(".ToolBarBackground").show();
        window.$RSjQuery(reportViewer).find(".ToolBarBackground").css({
            zoom: 1
        });
        toolbarButtons.each(function (index, button) {
            window.$RSjQuery(button).show();
        });

        // Scale icons if screen realestate is very small
        var zoomFactor = 1;
        if (availableSpace < 300) {
           window.$RSjQuery(reportViewer).find(".ToolBarBackground").hide();
        }
        ...
        else if (availableSpace < 700) {
           zoomFactor = 0.85;
        }
        else if (availableSpace < 800) {
           zoomFactor = 0.9;
        }

        // Zoom the toolbar background but scale up the div by the zoom factor so it still fits the parent 100%
        window.$RSjQuery(reportViewer).find(".ToolBarBackground").css({
           zoom: zoomFactor
        });
        window.$RSjQuery(reportViewer).find(".ToolBarBackground").width(window.$RSjQuery(reportViewer).width() / zoomFactor);

        var postScaleAvailableSpace = window.$RSjQuery(reportViewer).find(".ToolBarButtonsCell").width() - 100 /* leave some space to the right */;
        var postScaleWithOfIcons = 0;

        toolbarButtons.each(function (index, button) {
           // Report Viewer Control in VS uses html style tags to override the background color
           // styles actually have a width and shouldn't be included in the hidding logic
           if (!window.$RSjQuery(button).is("style")) {
               postScaleWithOfIcons += window.$RSjQuery(button).outerWidth(true /* include margin */);
           }
        });

        // Hide anything that doesn't fit
        window.$RSjQuery(toolbarButtons.get().reverse()).each(function (index, button) {
           if (postScaleWithOfIcons >= postScaleAvailableSpace) {
               // Report Viewer Control in VS uses html style tags to override the background color
               // styles actually have a width and shouldn't be included in the hidding logic
               if (!window.$RSjQuery(button).is("style")) {
                   postScaleWithOfIcons -= window.$RSjQuery(button).outerWidth(true /* include margin */);
                   window.$RSjQuery(button).hide();
               }
           }
        });
    },
    ...
    ConnectEventHandlers: function ()
    {
        ...
        // Power BI features
        if (this.m_powerBIFeaturesButton != null) {
            this.m_powerBIFeaturesButton.control.SetOnClickHandler(true, this.m_onPowerBIFeaturesPressDelegate);
        }
    ...
    EnableDisable: function (forEnable)
    {
        ...
        if (!forEnable || !Microsoft.Reporting.WebFormsClient._ReportAreaAsyncLoadTarget.AsyncTriggerSelected)
        {
            ...
            this.EnableDisableImage(this.m_powerBIFeaturesButton, isDisplayingReportPage);
        }
    },
    ...
    UpdateForNewReportPage: function (updateProperties)
    {
        ...
        // send a message to parent to inform about report action
        var eventData = {
            event: "UpdateForNewReportPageEvent",
            message: this.CurrentPage
        };
        parent.postMessage(eventData, "*");
    },
    ...
    OnPowerBIFeaturesPress: function ()
    {
        if (RS != null)
            RS.startPinning(this);
    },
    ...
    Utils: {
        ...
        readCookie: function (name) {
            ...
            for (var i = 0; i < ca.length; i++) {
                var c = ca[i];
                // string.trim is missing in ie 9-
...
Microsoft.Reporting.WebFormsClient.ReportViewer.prototype =
{
    ...
    invokePrintDialog: function()
    {
        ...
        return internalViewer.PdfPrint();
    },
...
Microsoft.Reporting.WebFormsClient._Splitter.prototype = {
    ...
    _setImage: function(hovering)
    {
        ...
        var imgsrc = null;
        var tooltip = collapsed ? this.TooltipExpand : this.TooltipCollapse;
        if (hovering)
        {
        ...
        this._image.title = tooltip;
        this._image.alt = tooltip;
    },
...
Microsoft.Reporting.WebFormsClient.ResizableControlBehavior.prototype = {
    ...
    _isRTL: function(control)
    {
        ...
        if (control instanceof Document) {
            control = control.documentElement;
        }
...
Microsoft.Reporting.WebFormsClient._AsyncWaitControl = function(element)
{
    ...
    this.m_waitFocusedFirstTime = false;
}
...
Microsoft.Reporting.WebFormsClient._AsyncWaitControl.prototype =
{
            ...
            if (!isLoading)
            {
                ...
                if (!showTrans) {
                    if (window.parent !== undefined) {
                        var msg = { event: "LoadingFinished" };
                        window.parent.postMessage(msg, "*");
                    }
                } 

                this.set_visible(showTrans, false, false);
                this._clearDelayTimer();
                this._clearPollingTimer();
            }
            else
    ...
    {
        this.set_visible(true, this._triggeringControlWasForThisViewer(), false);
    },

    set_visible: function(transVisible, waitVisible, waitFocusedFirstTime)
    {
        /// <summary>
        ...
        this.m_waitFocusedFirstTime = waitFocusedFirstTime;
    ...
    _start_visibility_polling: function()
    {
        ...
        // First time set focus, then let focus travel naturally.
        // Don't reset every time we poll, because then users cannot reach the cancel button via the keyboard.
        this.m_waitControl.focus();
        ...
        if (this.m_reportViewer.get_isLoading())
        {
            ...
            // shorter timeout = less smooth, but spinny stays in the wrong place shorter
    ...
    _set_visible_core: function () {
        ...
        if (!isStandardsMode && transVisible)
        {
            this._clip(element);
        }
        ...
        if (waitVisible)
        {
            ...
            if (!waitFocusedFirstTime)
            {
                // First time set focus, then let focus travel naturally.
                // Don't reset every time we poll, because then users cannot reach the cancel button via the keyboard.
                this.m_waitControl.focus();
                this.m_waitFocusedFirstTime = true;
            }
        }
    },
...
Microsoft.Reporting.WebFormsClient._DocMapArea.prototype =
{
    ...
    OnTextNodeEnter : function(textNode)
    {
        ...
        {
            textNode.className = "documentMapHighlighting";
        }
    },
    ...
    MarkNodeAsSelected : function(treeNode)
    {
        ...
        if (treeNode != null)
        {
            ...
            selectedTextNode.className = "documentMapHighlighting";
